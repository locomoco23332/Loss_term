        ...
        for ep in range(1, vae_epochs + 1):
            ...
            for step, indices in enumerate(sampler):
                ...
                for offset in range(iteration):
                    ...
                    output ,_,_,out = ae(history[:,condition_frame1-1,:],history[:,condition_frame1-2,:],history[:,condition_frame1-3,:],history[:,condition_frame1-4,:],history[:,condition_frame1-5,:],history[:,condition_frame1-6,:],history[:,condition_frame1-7,:],history[:,condition_frame1-8,:],history[:,condition_frame1-9,:],history[:,condition_frame1-10,:])
                    ...
                    B_loss=F.binary_cross_entropy(out_sig,ground_sig)
                    ...
                    #recon_loss3 = F.mse_loss(out2,ground_truth)
                    ...
                    #loss=recon_loss*0.85+recon_loss2*0.1+B_loss*0.05
            ...
            torch.save(copy.deepcopy(ae).cpu(), "TDG2GAVAE10_los.pt")
